...
import kotlinx.android.synthetic.main.activity_freeze.tabLayout

class FreezeActivity : AppCompatActivity() {

    override fun onCreateOptionsMenu(menu: Menu?): Boolean {
        val menuInflater = menuInflater
        menuInflater.inflate(R.menu.freeze_menu, menu)
        return super.onCreateOptionsMenu(menu)
    }

    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            R.id.freezeEditTabBtn -> {
                val editTabIntent = Intent(this, EditTabActivity::class.java)
                editTabIntent.putExtra("loc", 0)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // 목록 추가 버튼 클릭시
        freezeAddlistBtn.setOnClickListener {
            val addIntent = Intent(this, AddListActivity::class.java)
            addIntent.putExtra("loc",0)
        ...
        //
        // 1. tab 데이터 로딩
        var tabData = loadTabData("freeze")
        ...
        // 2. 어댑터 생성 및 데이터 전달
        val viewPagerAdapter = CustomViewPagerAdapter(this, "freeze", tabData)
        // 3. 화면에 있는 뷰페이저에 어댑터 연결
        viewPager0.adapter = viewPagerAdapter
        //
        //
        //*************************************************************//
        TabLayoutMediator(tabLayout, viewPager0) { tab, position ->
